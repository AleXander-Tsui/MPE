...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for i, agent in enumerate(world.agents):
            #agent.color = np.array([0.3*(i+1), 0.25*(i+1), 0.55*(i+1)]) #0.35 0.35 0.85
            agent.color = np.array([0.35,0.35,0.85]) #0.35 0.35 0.85
        # random properties for landmarks
        for i, landmark in enumerate(world.landmarks):
            #landmark.color = np.array([0.3*(i+1), 0.25*(i+1), 0.85*(i+1)]) #0.25 0.25 0.25
    ...
    def observation(self, agent, world):
        ...
        for entity in world.landmarks:  # world.entities:
            ...
            #entity_pos.append(np.concatenate((entity.state.p_pos - agent.state.p_pos, np.array([i]))))
        # entity colors
        entity_color = []
        for entity in world.landmarks:  # world.entities:
